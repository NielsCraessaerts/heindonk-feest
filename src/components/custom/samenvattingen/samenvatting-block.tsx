...
export default function SamenvattingBlock() {
  return (
    <section className="summary-anim relative overflow-hidden bg-[#2E5DAA] text-white">
      {/* Poster wash + stripes */}
      ...
      <div className="relative mx-auto w-full max-w-6xl px-6 py-12 md:py-16">
        <div className="text-center text-sm uppercase tracking-[0.4em] text-white/80">Programma</div>

        {/* Mobile: swiper */}
        <div className="mt-8 md:hidden">
          <Swiper spaceBetween={20} slidesPerView={1.1} centeredSlides grabCursor loop>
            {summaries.map((summary) => (
              <SwiperSlide key={summary.slug} className="!h-auto">
                <div className="flex min-h-[520px] flex-col text-center" data-anim>
                  <Link href={summary.slug} className="inline-block">
                    <p className="text-3xl font-bold uppercase tracking-[0.18em]">{summary.title}</p>
                  ...
                  <p className="mt-1 font-lobster text-4xl font-normal normal-case tracking-[0.06em]">
                  ...
                  <Link
                    href={summary.slug}
                    className="mt-auto inline-block rounded-full border border-white/60 px-6 py-2 text-[11px] font-semibold uppercase tracking-[0.3em] text-white/90 transition hover:bg-white/10"
                  >
                    Meer info
                  </Link>
                </div>
        ...
        <div className="mt-10 hidden gap-10 md:grid md:grid-cols-3">
          {summaries.map((summary) => (
